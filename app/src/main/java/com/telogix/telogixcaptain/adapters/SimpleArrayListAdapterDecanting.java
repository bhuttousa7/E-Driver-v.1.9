...
public class SimpleArrayListAdapterDecanting extends ArrayAdapter<Datum> implements Filterable, ISpinnerSelectedView {
    ...
    public SimpleArrayListAdapterDecanting(Context context, ArrayList<Datum> strings) {
    ...
    public long getItemId(int position) {
        if (mStrings == null && position > 0)
            return mStrings.get(position-1).getDecantingSiteID();
        else
            return -1;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == 0) {
            ...
            letters.setImageDrawable(getTextDrawable(mStrings.get(position-1).getDecantingSite()));
            dispalyName.setText(mStrings.get(position-1).getDecantingSite());
        }
        return view;
    }
    ...
    public class StringFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            for (Datum text : mBackupStrings) {
                if (text.getDecantingSite().toLowerCase().contains(charString.toLowerCase())) {
